...
| `cheats/file_backed.py` | keeps the blocks in a file in the working directory, `/tmp`, `/var/tmp`, `/dev/shm`, the home directory and `TMPDIR`, and in a daemon behind an abstract socket, and reloads them at MOUNT | 636 integrity violations, identical to keeping them in memory; the report records 132 files and 46 processes removed from it across the ten scenarios, reward 0 |
...
It now scores 636 integrity violations, exactly what the memory only store scores, and the run report
