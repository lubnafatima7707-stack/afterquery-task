...
wrote `1` to `/logs/verifier/reward.txt` with `6 passed in 20.62s`, and produced
...
runs the three development scenarios and prints the same measurements: 0 violations, 1.92 ms worst
mount, 0 overruns, 2 ticks worst acknowledgement, 2.376 erases per 100 writes, `bars: all met`.

## Determinism

The verifier was run three times over the reference output. All three gave `6 passed` and identical
numbers: 0 violations, 2.380 ms worst mount, 0 tick overruns, 4 ticks worst acknowledgement, 0
writes left unacknowledged, 2.384 erases per 100 writes, 59 boots, 124588 ticks. Regenerating the
...
| `solution/nvm_store.py` | 0 | 2.380 | 0 | 4 | 0 | 2.384 |
| `independent_store.py` | 0 | 4.020 | 0 | 7 | 0 | 3.110 |
| worst passing perturbation | 0 | 2.540 | 0 | 35 | 0 | 3.872 |
| nearest failing variant | 3 | 26.625 | 1763 | 461 unacked | | 6.267 |
...
spends on summaries; the two land 0.7 erases per 100 writes apart, and every bar sits between the
weaker of them and the nearest variant that fails.

## What each bar is the sole reason for

- integrity: believing a page on its tag alone (3 violations and a crash), applying the sectors at
  mount in sector order rather than generation order (1000), acknowledging on arrival (6)
- mount: dropping the per sector summaries (27.875 ms), and in the independent store dropping the
  checkpoint (26.625 ms)
- tick budget: doing every job in the tick it arrives in (1763 overruns)
- acknowledgement: keeping one blank sector rather than two, giving a sector up after a single
  erase, or closing the open sector ten pages early (461 writes stranded in each case)
- endurance: holding six sectors blank rather than two (6.267 erases per 100 writes)
...
26.625 ms of mount without the checkpoint, 70 violations with 504 writes stranded without the erase
proof, 4 violations acknowledging on arrival, and 1 violation with 1106 writes stranded with one
blank sector instead of two.
...
one with the fewest live records passes, at 2.562 erases per 100 writes against the reference's
2.384. That is a property of this workload rather than a gap in the bar. Fresh writes and reclaimed
...
erase: closing the open sector twenty pages early reaches 8.082, and holding six sectors blank
reaches 6.267.
